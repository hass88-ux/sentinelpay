...
Open registration, a second real Google account, cancellation, and sign-out/re-login still need live acceptance checks. Google remains in Testing pending branding and publishing requirements. My uploads now identifies its private workspace separately from the demo. The saved-file list distinguishes loading, failed requests, and an actual empty result; Refresh files retries failed requests.
