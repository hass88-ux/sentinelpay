...
if ($Verify -and $IncidentDemo) { throw 'Choose verification or the incident demo, not both.' }
if ($ExportDashboard -and ($Verify -or $IncidentDemo)) { throw 'Dashboard export must run on its own.' }
...
try {
    ...
    $mode = if ($ExportDashboard) { '--export-dashboard' } elseif ($Verify) { '--verify' } elseif ($IncidentDemo) { "--incident-demo=$Port" } else { "$Port" }
