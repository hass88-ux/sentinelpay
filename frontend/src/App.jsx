...
export default function App() {
  ...
  return <div className="app"><aside className="sidebar"><a className="brand" href="#" onClick={e=>{e.preventDefault();setView('Overview')}}><img src="/favicon.svg" alt=""/>SentinelPay</a>
    ...
    <div className="sidebar-footer"><span className="system-mark">SP</span><div>{view==='My uploads'?'Private workspace':source==='demo'?'Demo workspace':'Local workspace'}<small>{view==='My uploads'?'Your saved payment files':'Java · Kafka · PostgreSQL'}</small></div></div></aside>
    <main><header><div className="breadcrumb"><strong>SentinelPay</strong> <span>/</span> {view}</div><div className="header-controls">{view==='My uploads'?<span className="source-badge">Private uploads</span>:<><label className="sr-only" htmlFor="source">Data source</label>{liveAvailable?<select id="source" value={source} onChange={e=>setSource(e.target.value)}><option value="demo">Saved demo</option><option value="live">Live pipeline</option></select>:<span className="source-badge">Saved demo</span>}</>}</div></header>
    ...
    <footer>SentinelPay <span>{data?`Snapshot ${new Date(data.capturedAt).toLocaleString('en-GB',{timeZone:'UTC'})} UTC`:'Evidence first'}</span></footer></div><footer><a href="/privacy.html">Privacy policy</a><a href="/terms.html">Terms of use</a></footer></main></div>;
}
