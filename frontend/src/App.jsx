...
export default function App() {
  ...
  const [view,setView]=useState('Overview'),[data,setData]=useState(null),[currency,setCurrency]=useState('USD');
  ...
  async function ask(text=question) {
    ...
    try {
      const result=source==='demo'?savedAnswer(data,selected,trimmed):await request(`/api/intelligence/incidents/${encodeURIComponent(selected)}/questions`,{method:'POST',body:JSON.stringify({question:trimmed}),signal:controller.signal});
  ...
  return <div className="app"><aside className="sidebar"><a className="brand" href="#" onClick={e=>{e.preventDefault();setView('Overview')}}><img src="/favicon.svg" alt=""/>SentinelPay</a>
    <div className="workspace">PAYMENT OPERATIONS</div><nav aria-label="Main navigation">{['Overview','Transactions','Incidents'].map((name,i)=><button key={name} aria-current={view===name?'page':undefined} className={view===name?'active':''} onClick={()=>setView(name)}><span aria-hidden="true">{['◫','⇄','◇'][i]}</span>{name}</button>)}</nav>
    ...
    <main><header><div className="breadcrumb">Workspace <span>/</span> {view}</div><div className="header-controls"><label className="sr-only" htmlFor="source">Data source</label>{liveAvailable?<select id="source" value={source} onChange={e=>setSource(e.target.value)}><option value="demo">Saved demo</option><option value="live">Live pipeline</option></select>:<span className="source-badge">Saved demo</span>}</div></header>
    <div className="page"><div className="title-row"><div><p className="eyebrow">{view==='Overview'?'SYSTEM OVERVIEW':view==='Transactions'?'PAYMENT ACTIVITY':'INCIDENT INVESTIGATION'}</p><h1>{view==='Overview'?'Payment health':view==='Transactions'?'Transactions':'Incident inbox'}</h1><p className="muted">{view==='Overview'?'Metrics, signals, and the evidence behind them.':view==='Transactions'?'Inspect the latest stored payment attempts.':'Follow the evidence from alert to explanation.'}</p></div><div className="title-controls"><label className="sr-only" htmlFor="currency">Currency</label><select id="currency" value={currency} onChange={e=>{setCurrency(e.target.value);setSelected(null)}}>{currencies.map(c=><option key={c}>{c}</option>)}</select><button className="button" disabled={loading||!!action} onClick={()=>setRefresh(n=>n+1)}>{loading?'Loading…':'↻ Refresh'}</button></div></div>
    <div className="demo-banner"><strong>{source==='demo'?'Demo dataset':'Live pipeline'}</strong><span>{source==='demo'?'Saved synthetic traffic. Questions use recorded backend answers.':'Reads the Java backend. Counts reflect the returned metric window.'}</span><span className="mono">{currency} · UTC</span></div>
    ...
    <div className="question-box"><div className="question-heading"><h3>Ask about this incident</h3><span>{source==='demo'?'Saved answers':'Evidence-based answers'}</span></div><p className="muted">{source==='demo'?'Explore answers recorded by the Java backend.':'Questions use this case only. AI is optional; basic answers work without a model.'}</p><div className="suggestions">{starterQuestions.slice(0,3).map(text=><button key={text} disabled={asking} onClick={()=>ask(text)}>{text}</button>)}</div><form onSubmit={e=>{e.preventDefault();ask()}}><label className="sr-only" htmlFor="question">Incident question</label><textarea id="question" value={question} onChange={e=>setQuestion(e.target.value)} maxLength={500} rows={3} placeholder="What should I investigate next?"/><div className="question-submit"><span>{question.length}/500 · One case, no saved chat</span><button className="button primary" disabled={asking||!question.trim()}>{asking?'Getting answer…':'Ask question'}</button></div></form>{questionError&&<p className="notice" role="alert">{questionError}</p>}{answer&&<div className="answer" role="status"><span className="answer-label">{answer.mode==='SAVED_DEMO'?'DEMO LIMITATION':source==='demo'?'SAVED BACKEND RESPONSE':answer.mode==='AI_ASSISTED'?'AI SUGGESTION · UNVERIFIED':'RULE-BASED RESPONSE'}</span><p>{answer.answer}</p>{answer.evidenceRules?.length>0&&<div className="citations">{answer.evidenceRules.map(r=><span key={r}>{ruleLabels[r]??r}</span>)}</div>}{answer.explanation?.notice&&<small>{answer.explanation.notice}</small>}</div>}</div><details className="limitations"><summary>Context and limits</summary><ul>{report.limitations.map(text=><li key={text}>{text}</li>)}</ul><p>Case ID: <code>{report.incident.id}</code></p></details></div>}</section>}</div>}
    </>}
