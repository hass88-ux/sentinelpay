...
export default function Chart({metrics,signal}) {
  ...
  return <><div className="chart"><svg viewBox="0 0 660 250" role="img" aria-label={`${signal==='latency'?'Average latency in milliseconds':signal==='failure'?'Failure percentage':'Payment volume'} across ${metrics.length} observed UTC minutes`}>
    <defs><linearGradient id="area" x1="0" y1="0" x2="0" y2="1"><stop stopColor="#16a58d" stopOpacity=".18"/><stop offset="1" stopColor="#16a58d" stopOpacity="0"/></linearGradient></defs>
    {[0,.25,.5,.75,1].map(f=><g key={f}><line x1="52" x2="632" y1={210-f*175} y2={210-f*175} stroke="#e7ecf2"/><text x="0" y={214-f*175}>{number(max*f)}</text></g>)}
    {warning!==null&&<line x1="52" x2="632" y1={210-warning/max*175} y2={210-warning/max*175} stroke="#d89435" strokeDasharray="5 5"/>}
    {segments(metrics).map((part,i)=><g key={i}><path d={`M ${x(part[0])} 210 ${part.map(m=>`L ${x(m)} ${y(m)}`).join(' ')} L ${x(part.at(-1))} 210 Z`} fill="url(#area)"/><polyline points={part.map(m=>`${x(m)},${y(m)}`).join(' ')} fill="none" stroke="#159b85" strokeWidth="3" strokeLinejoin="round"/>
    {part.map(m=><circle key={m.bucket} cx={x(m)} cy={y(m)} r="4" fill="#159b85"><title>{clock(m.bucket)} UTC: {number(value(m),1)}</title></circle>)}</g>)}
