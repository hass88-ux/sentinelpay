...
Private accounts use Supabase Google sign-in and a protected Java API on Render. Google OAuth is published in Production, with privacy and terms pages linked from the site and Google's sign-in screen. Browser checks confirmed sign-out, Google re-login, a saved 180-payment CSV, persistence after reload, matching metrics and threshold checks, transaction filtering, and a fresh live AI response. A second Google account opened an empty private workspace and successfully saved its own sample file. Private-upload AI is live with explicit consent, owner-authorized aggregate evidence, and persistent per-account limits. A live Groq answer correctly described the sample's failure/latency spike and subsequent volume drop, citing the corresponding rules. See the [Google sign-in guide](docs/google-sign-in.md) and [private account setup](docs/private-accounts.md).
